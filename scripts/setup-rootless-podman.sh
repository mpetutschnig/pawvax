...
EOF
  ...
  # Append derived variables that quadlet EnvironmentFile= passes into containers directly
  local db_pass
  db_pass=$(grep '^DB_PASSWORD=' "$ENV_SOURCE" | cut -d= -f2-)
  cat >> "$app_home_dir/.config/pawvax/paw.env" << ENVEOF

# Derived — auto-generated by setup script, do not edit manually
...
prepare_host() {
  ...
  mkdir -p "$app_home_dir/data/postgres" "$app_home_dir/data/uploads" \
    "$app_home_dir/.config/systemd/user" "$app_home_dir/.config/containers/systemd" \
    "$app_home_dir/.local/share/containers"
  ...
  chown -R "$APP_USER:$APP_USER" "$app_home_dir/.config/systemd" "$app_home_dir/.config/containers" "$app_home_dir/.config/pawvax" 2>/dev/null || true
...
write_quadlet_files() {
  local app_home_dir
  app_home_dir="$(app_home)"
  local quadlet_dir="$app_home_dir/.config/containers/systemd"

  # Pod — publishes ports and owns the shared network namespace
  cat > "$quadlet_dir/paw-stack.pod" << EOF
[Unit]
Description=PAW Application Stack Pod
...
EOF
  ...
  # PostgreSQL
  cat > "$quadlet_dir/paw-postgres.container" << EOF
[Unit]
Description=PAW PostgreSQL Database
After=network-online.target

[Container]
Image=docker.io/postgres:16-alpine
ContainerName=paw-postgres
Pod=paw-stack.pod
EnvironmentFile=%h/.config/pawvax/paw.env
Volume=%h/data/postgres:/var/lib/postgresql/data:Z,U
...
EOF
  ...
  # API server
  cat > "$quadlet_dir/paw-api.container" << EOF
[Unit]
Description=PAW API Server
After=network-online.target paw-postgres.service
Requires=paw-postgres.service
...
EOF
  ...
  # PWA (nginx static)
  cat > "$quadlet_dir/paw-pwa.container" << EOF
[Unit]
...
EOF
  ...
  # Caddy reverse proxy
  cat > "$quadlet_dir/paw-caddy.container" << EOF
[Unit]
Description=PAW Caddy Reverse Proxy
After=network-online.target paw-api.service paw-pwa.service

[Container]
Image=localhost/paw-caddy:latest
ContainerName=paw-caddy
Pod=paw-stack.pod
...
EOF
  ...
  chown "$APP_USER:$APP_USER" \
    "$quadlet_dir/paw-stack.pod" \
    "$quadlet_dir/paw-postgres.container" \
    "$quadlet_dir/paw-api.container" \
    "$quadlet_dir/paw-pwa.container" \
...
deploy_stack() {
  ...
  local pg_pid_file="$app_home_dir/data/postgres/postmaster.pid"
  ...
  run_as_app "
    ...
    until podman exec paw-postgres pg_isready -U '$DB_USER' >/dev/null 2>&1; do
      i=\$((i+1))
      [ \$i -ge 60 ] && { echo 'ERROR: PostgreSQL did not become ready in time.' >&2; exit 1; }
      sleep 3
    done
...
cleanup_stack() {
  ...
  rm -f \
    "$app_home_dir/.config/containers/systemd/paw-stack.pod" \
    "$app_home_dir/.config/containers/systemd/paw-postgres.container" \
    "$app_home_dir/.config/containers/systemd/paw-api.container" \
    "$app_home_dir/.config/containers/systemd/paw-pwa.container" \
    "$app_home_dir/.config/containers/systemd/paw-caddy.container"
...
status_stack() {
  ...
  run_as_app "systemctl --user status \
    'paw-postgres.service' 'paw-api.service' 'paw-pwa.service' 'paw-caddy.service' \
    --no-pager || true"
}
