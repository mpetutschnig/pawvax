...
export async function initDb(connectionString) {
  ...
  try { await pool.query("ALTER TABLE voice_memos ADD COLUMN IF NOT EXISTS gladia_debug_json TEXT") } catch { /* already exists */ }
  try { await pool.query("ALTER TABLE voice_memos ADD COLUMN IF NOT EXISTS ai_debug_json TEXT") } catch { /* already exists */ }
